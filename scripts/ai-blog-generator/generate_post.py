#!/usr/bin/env python3
"""
AI Blog Post Generator — Habit, Countdown & Self-Improvement Blog.
...
def generate_topic(client, existing_context: str, skill_prompt: str) -> str:
    """Use AI to generate a unique blog topic about habits/self-improvement."""
    response = retry_api_call(lambda: client.models.generate_content(
        model="gemini-flash-latest",
        contents=f"""Generate ONE new blog post topic for an English self-improvement blog about habits, countdowns, streaks, and personal growth.

{existing_context}

IMPORTANT: The new topic MUST be different from ALL existing posts listed above.
Choose a topic that would attract readers interested in: quitting bad habits, building good habits, streak tracking, countdowns, goal setting, life reset, productivity, routines, personal growth, relationship milestones, or 30-day challenges.""",
        config=types.GenerateContentConfig(
            system_instruction=f"""You are a content strategist for an English self-improvement blog about habits, countdowns, and personal growth.
...
{{
  ...
  "content_type": "how-to|listicle|myth-busting|life-reset|beginners-guide|30-day-challenge|what-happens-after|relationship",
...
- Pick topics that naturally lead to discussing habit tracking, streaks, countdowns, or self-improvement
- Vary content types — don't always pick the same type
- Rotate between content categories: quit bad habits, build good habits, habit psychology, motivation, streak tracking, countdowns, challenges, life reset, productivity, goal setting, relationships, personal growth
- Consider evergreen reference articles that bring repeat visitors""",
            temperature=0.9,
...
- Tags should be lowercase English from this list: habits, self-improvement, motivation, streak, days-since, countdown, goal-tracking, productivity, routine, discipline, consistency, life-reset, personal-growth, challenges, relationship, anniversary, goals
- image_prompts: MUST include banner, inline1, and inline2 (3 images total)
- banner: MUST directly represent the specific topic of the blog post (e.g., if the post is about morning routines, the image should depict a morning scene with healthy habits). Do NOT use generic motivational images.
- All image prompts should describe clean, modern, lifestyle-themed illustrations (no text in images, no faces)
...
def generate_content(client, topic: str, title: str, skill_prompt: str, existing_context: str) -> str:
    ...
    response = retry_api_call(lambda: client.models.generate_content(
        ...
        config=types.GenerateContentConfig(
            system_instruction=f"""You are an English self-improvement blog writer specializing in habits, countdowns, streaks, and personal growth.
...
- Follow the soft CTA strategy: naturally weave in mentions of tracking progress, streaks, countdowns — max 3 per article
- Do NOT mention any specific third-party app names. Use generic terms like "habit tracking apps" or "countdown tools" instead.
- Do NOT create internal links to other blog posts (no /blog/... links). Each post must be self-contained.
- End with a "Key Takeaways" section summarizing 3-5 main points with a soft CTA woven in
- Include psychology or reasoning to support advice
- Include real-world examples and relatable situations
- When discussing mental health or addiction, include disclaimer: "If you're struggling, please reach out to a professional or a trusted person in your life."
- Write like a human — conversational, varied sentence length, no AI-sounding phrases""",
            temperature=0.8,
        ),
    ))
...
def main():
    print("🚀 Blog Generator — Habit, Countdown & Self-Improvement")
    print("=" * 60)
    sys.stdout.flush()
    ...
    print("\n" + "=" * 60)
    print("✅ Done! New blog post created successfully.")
    sys.stdout.flush()
